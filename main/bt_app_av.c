...
{
    static uint8_t adjusted[2048];  // buffer temporaire, peut être alloué dynamiquement si besoin
    uint8_t volume;
    _lock_acquire(&s_volume_lock);
    volume = s_volume;
    _lock_release(&s_volume_lock);

    float gain = (float)volume / 0x7f; // volume entre 0.0 et 1.0

    for (uint32_t i = 0; i < len; i++) {
        int16_t sample = (int16_t)((int)data[i] - 128); // centrer autour de 0
        sample = (int16_t)(sample * gain);             // appliquer le gain
        adjusted[i] = (uint8_t)(sample + 128);         // recentrer
    }

    write_ringbuf(adjusted, len);
